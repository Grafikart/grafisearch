...
const injectResult = (selector: string) => (results: SearchResult[]) => {
	...
	container.innerHTML = results
		.map((r) => {
            ...
            <div>
              <a tabindex="-1" rel="noopener noreferrer" class="result__title" href="${
								r.url
							}">${r.title}</a>
            ...
            <a class="result__link" rel="noopener noreferrer" href="${
							r.url
						}"></a>
      ...
      <a tabindex="-1" class="result__title" rel="noopener noreferrer" href="${
				r.url
			}">${r.title}</a>
      <div tabindex="-1" class="result__url">
          <img src="${favicon}" alt="">
...
function logButton(found: boolean, url: string) {
	return `<a rel="noopener noreferrer" href="${url}" class="btn-icon results-confirm" data-found="${Number(
		found
	)}">
