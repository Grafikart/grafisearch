...
let currentTimer: {duration: number, end: number} | null = null;
...
function updateTimer() {
  ...
  const secondes = Math.round((ms / 1000) % 60).toString().padStart(2, "0")
  headerText.innerText = `${minutes}:${secondes}`
  header.classList.add('has-timer')
  header.style.setProperty("--progress", (ms / currentTimer.duration).toFixed(4))
  if (ms > 0) {
